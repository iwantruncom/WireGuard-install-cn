...
function installQuestions() {
	...
	read -rp "服务器公网 IP 地址：" -e -i "${SERVER_PUB_IP}" SERVER_PUB_IP

	SERVER_NIC="$(ip -4 route ls | grep default | awk '/dev/ {for (i=1; i<=NF; i++) if ($i == "dev") print $(i+1)}' | head -1)"
	until [[ ${SERVER_PUB_NIC} =~ ^[a-zA-Z0-9_]+$ ]]; do
		read -rp "公网网卡名称：" -e -i "${SERVER_NIC}" SERVER_PUB_NIC
	done
	...
	until [[ ${SERVER_WG_IPV4} =~ ^([0-9]{1,3}\.){3} ]]; do
		read -rp "服务端内网 IP（VPN 网段）：" -e -i 10.66.66.1 SERVER_WG_IPV4
	done

	RANDOM_PORT=$(shuf -i49152-65535 -n1)
	until [[ ${SERVER_PORT} =~ ^[0-9]+$ ]] && [ "${SERVER_PORT}" -ge 1 ] && [ "${SERVER_PORT}" -le 65535 ]; do
		read -rp "WireGuard 监听端口 [1-65535]：" -e -i "${RANDOM_PORT}" SERVER_PORT
	done

	until [[ ${CLIENT_DNS_1} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "首选 DNS：" -e -i 1.1.1.1 CLIENT_DNS_1
	done
	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "备用 DNS：" -e -i 8.8.8.8 CLIENT_DNS_2
	done
...
function newClient() {
	...
	echo ""
	echo "客户端名称（字母、数字、下划线、横线，最多 15 个字符）："
	...
	until [[ ${IPV4_EXISTS} == '0' ]]; do
		read -rp "客户端内网 IP：${BASE_IP}." -e -i "${DOT_IP}" DOT_IP
...
AllowedIPs = ${CLIENT_WG_IPV4}/32" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
	echo ""
	echo -e "  服务器：${SERVER_PUB_IP}  端口：${SERVER_PORT}/UDP  客户端IP：${CLIENT_WG_IPV4}"
	echo ""
	...
	echo ""
	echo -e "  WireGuard 客户端：https://www.wireguard.com/install"
	echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
}
