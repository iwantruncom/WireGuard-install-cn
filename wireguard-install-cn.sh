...
function showQRCode() {
	...
	# 首选方案：优先使用系统自带的 qrencode (速度最快，兼容性最好)
	if command -v qrencode &>/dev/null; then
		qrencode -t ansiutf8 < "${CONF_FILE}"
		return 0
	fi

	# 备用方案：如果系统极度精简没有 qrencode，则尝试使用 Python 渲染
	local PYTHON_CMD="python3"
	if ! command -v ${PYTHON_CMD} &>/dev/null; then
		if command -v python &>/dev/null; then
			PYTHON_CMD="python"
		else
			echo -e "  ${RED}无法生成二维码：未找到 qrencode 或 Python 环境${NC}"
			return 1
		fi
	fi
...
function installQuestions() {
	...
	echo -e "  感谢张狗剩同志 https://x.com/goshenggo 催更"
	echo ""
...
function installWireGuard() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
		apt-get update
		apt-get install -y wireguard iptables resolvconf qrencode python3 python3-pip
	elif [[ ${OS} == 'debian' ]]; then
		...
		apt-get update
		apt-get install -y iptables resolvconf qrencode python3 python3-pip
		...
		dnf install -y wireguard-tools iptables qrencode python3 python3-pip
	elif [[ ${OS} == 'centos' ]] || [[ ${OS} == 'almalinux' ]] || [[ ${OS} == 'rocky' ]]; then
		if [[ ${VERSION_ID} == 8* ]]; then
			yum install -y epel-release elrepo-release
			yum install -y kmod-wireguard
		fi
		yum install -y wireguard-tools iptables qrencode python3 python3-pip
	elif [[ ${OS} == 'oracle' ]]; then
		...
		pacman -S --needed --noconfirm wireguard-tools qrencode python python-pip
	elif [[ ${OS} == 'alpine' ]]; then
		apk update
		apk add wireguard-tools iptables qrencode python3 py3-pip
	fi
...
AllowedIPs = ${CLIENT_WG_IPV4}/32" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	echo ""
	echo -e "  Windows 下载："
	echo -e "  · WinSCP（推荐新手）：https://winscp.net 使用服务器账号密码登录后，找到上方路径下载到本地电脑即可"
